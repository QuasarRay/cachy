...
tmp_dir=$(mktemp -d)
cleanup() {
  # SquashFS intentionally restores source permissions. Cleanup must never
  # replace the validator's real exit status with an rm permission failure.
...
if ! unsquashfs -f -d "$tmp_dir/rootfs" "$rootfs" var/cache/pacman/pkg >/dev/null; then
  fail "failed to extract /var/cache/pacman/pkg from live SquashFS"
fi
repo="$tmp_dir/rootfs/var/cache/pacman/pkg"
[[ -d "$repo" ]] || fail "finished ISO has no embedded /var/cache/pacman/pkg directory"
...
echo "Embedded offline repository and checksum manifest are present"
...
grep -Fqx 'Server = file:///var/cache/pacman/pkg' "$tmp_dir/pacman-offline.conf" || \
  fail "offline repository file:// server missing from /etc/pacman-offline.conf"
