...
for cmd in bsdtar unsquashfs tar awk grep find sort sha256sum realpath; do
...
# Do not depend on loop devices or mount privileges. GitHub-hosted fresh
# runners can inspect ISO9660 with libarchive entirely in userspace.
...
if ! bsdtar -xOf "$iso" arch/x86_64/airootfs.sfs >"$rootfs"; then
  echo "ISO does not contain a readable arch/x86_64/airootfs.sfs" >&2
  exit 1
fi
[[ -s "$rootfs" ]] || {
  echo "ISO root filesystem image is empty" >&2
  exit 1
}
